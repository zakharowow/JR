...
public class Solution
{
    ...
    public static void cleanAllApartaments(List<Apartament> apartaments)
    {
        for(int i = 0; i < apartaments.size(); i++){
            if (apartaments.get(i) instanceof Apt1Room){new Apt1Room().clean1Room();}//написать тут вашу реализацию пунктов 1-4
            if (apartaments.get(i)instanceof Apt2Room){new Apt2Room().clean2Rooms();}
            if (apartaments.get(i) instanceof Apt3Room){new Apt3Room().clean3Rooms();}
        }
    }
